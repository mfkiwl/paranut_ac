...
#ifndef HAS_PRINTF
#define HAS_PRINTF 0
#define ee_printf(format, ...) \
    do \
    { \
        if (pn_get_cpuid() != 0) wait_for_cpu(pn_get_cpuid()-1); \
        printf("CPU %d: ", pn_get_cpuid()); \
...
#elif USE_PNTHREAD
	#define PARALLEL_METHOD "ParaNut Threads"
#else
...
#if USE_PNTHREAD
typedef enum {
    SIGNO_THREAD_CTXTINIT = 0x0,
    SIGNO_THREAD_START    = 0x1,
    SIGNO_THREAD_FINISHED = 0x2
} signo_thread_t;
#endif
...
typedef struct CORE_PORTABLE_S {
    ...
    #elif USE_PNTHREAD
    volatile signo_thread_t thread_sig;
	#endif /* Method for multithreading */
#endif /* MULTITHREAD>1 */
	ee_u8	portable_id;
} core_portable;
