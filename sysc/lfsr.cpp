...
TWord primePolyListFewXors [MAXLFSR - 1] [16] = {
  ...
  // degree 24...
  { 0x0000001b, 0x00000087, 0x0000a011, 0x00010281, 0x00028101, 0x00100a01, 0x00180003, 0x00800031, 
    0x00b00001, 0x00c20001, 0x001ba555, 0x002fe633, 0x00554bb1, 0x0098cfe9 },
...
TWord primePolyListManyXors [MAXLFSR - 1] [16] = {
...
TWord GetPrimePoly (int degree, int select) {
  select %= 32;
  if (select < 16) return primePolyListFewXors [degree - 2] [select % polysAvailable [degree - 2]];
  select -= 16;
  return primePolyListManyXors [degree - 2] [select % polysAvailable [degree - 2]];
}
...
TWord GetNextLfsrState (TWord state, int width, TWord poly) {
  ...
  oldBit0 = state & 1;
  if (oldBit0) state ^= poly;
  return (state >> 1) | (oldBit0 << (width - 1));
}
...
void DisplayLfsrTaps () {
  ...
  for (n = 2; n <= 32; n++) {
    printf ("%3i |", n);
    for (k = 0; k < polysAvailable [n-2]; k++) printf ("%4i", GetOnes (primePolyListManyXors [n-2] [k]) - 1);
    putchar ('\n');
  }
}
