...
struct SCacheTag {
  ...
  TWord tadr;    // requires 32 - CACHE_SETS_LD - CACHE_BANKS_LD - 2 bits
  TWord way;     // requires CACHE_WAYS_LD bits
...
#define TR_PORTS CPU_CORES  // see comment to 'MArbiter'


struct STagEntry {
  bool valid[CACHE_WAYS], dirty[CACHE_WAYS];
  TWord tadr[CACHE_WAYS];
...
public:
  ...
  sc_in<bool> rd[TR_PORTS], wr[TR_PORTS];   // write accesses require two clock cycles; 'wr', 'adr' and 'tag_in' must be asserted this long

  sc_in<TWord> adr[TR_PORTS];   // complete adress (lower CACHE_BANKS_LD + 2 bits can be omitted)
...
protected:
  TWord ram[CACHE_SETS * CACHE_WAYS];
};
...
public:
  ...
  sc_out<sc_uint<4> >  wb_sel_o;     // byte select outputs
  ...
  sc_in<TWord>         wb_dat_i;     // input data bus
  sc_out<TWord>        wb_dat_o;     // output data bus
  ...
  sc_out<bool> tag_rd, tag_wr, bank_rd[CACHE_BANKS], bank_wr[CACHE_BANKS];

  //   Adress & data busses...
  sc_in<TWord> adr_in;
  sc_out<TWord> adr_out;                // to cache banks, tag bank, and r/w ports to determine BUSIF hits
  sc_in<TWord> data_in[CACHE_BANKS];    // data from cache banks (bank_rd[n] = 1) or write ports (else)...
  sc_out<TWord> data_out[CACHE_BANKS];  // data read from bus
  sc_out<bool> data_out_valid[CACHE_BANKS];
  sc_in<SCacheTag> tag_in;
  sc_out<SCacheTag> tag_out;

  //   Request & grant lines...
  sc_out<bool> req_linelock, req_tagw, req_tagr, req_bank[CACHE_BANKS];
  sc_in<bool> gnt_linelock, gnt_tagw, gnt_tagr, gnt_bank[CACHE_BANKS];

  // Constructor...
  SC_CTOR(MBusIf) {
    SC_METHOD(OutputMethod);
      sensitive << adr_reg << tag_reg;
      for (int n = 0; n < CACHE_BANKS; n++) sensitive << idata_reg[n] << idata_valid_reg[n];
    SC_CTHREAD(MainThread, clk.pos());
      reset_signal_is (reset, true);
  }
...
protected:
  ...
  sc_signal<TWord> adr_reg;
  sc_signal<TWord> idata_reg[CACHE_BANKS], odata_reg[CACHE_BANKS];   // TBD: rename to "rdata...", "wdata..."
  sc_signal<bool> idata_valid_reg[CACHE_BANKS];
...
typedef enum {
  s_rp_init = 0, s_rp_direct_wait_busif = 1,
  s_rp_request_tag_only = 2, s_rp_read_tag = 3, s_rp_read_bank = 4,
  s_rp_miss_wait_busif = 5, s_rp_miss_request_tag = 6, s_rp_miss_read_tag = 7, s_rp_miss_replace = 8
} e_readport_state;
...
public:
  ...
  sc_in<bool> busif_data_valid[CACHE_BANKS];
  ...
  //   Request & grant lines...
  sc_out<bool> req_tagr, req_bank[CACHE_BANKS], req_busif;
  sc_in<bool>  gnt_tagr, gnt_bank[CACHE_BANKS], gnt_busif;

  // Constructor...
  SC_CTOR(MReadPort) {
    SC_METHOD(HitMethod);
      sensitive << port_adr << busif_adr << tag_in;
      for (int b = 0; b < CACHE_BANKS; b++) sensitive << busif_data_valid[b];
...
protected:
  ...
  // Registers...
  sc_signal<e_readport_state> state_reg;
  ...
  sc_signal<e_readport_state> next_state;
...
typedef enum {
  s_wp_init = 0, s_wp_direct = 1,
  s_wp_request_linelock_only = 2, s_wp_read_tag = 3, s_wp_have_tag_request_bank = 4,
  s_wp_read_bank = 5,
  s_wp_write_tag1_and_bank = 6, s_wp_write_tag2_and_bank = 7, s_wp_write_tag1 = 8, s_wp_write_tag2 = 9, s_wp_write_bank = 10,
  s_wp_miss = 11, s_wp_request_busif_only = 12,
  s_wp_recheck = 13, s_wp_recheck_read_tag = 14,
  s_wp_replace = 15, s_wp_replace_wait_busif = 16,
  s_wp_special_request_busif_only = 17, s_wp_special = 18, s_wp_special_wait_complete = 19
} e_writeport_state;
...
public:
  ...
  sc_in<bool> port_rlink_wcond;   // for LL/SC: link current adress (wr = 0) / store only when link is valid (wr = 1)
  sc_out<bool> port_wcond_ok;     // for LL/SC: set, if write-conditional was sucessful
  ...
  //   Request & grant lines...
  sc_out<bool> req_linelock, req_tagr, req_tagw, req_bank[CACHE_BANKS], req_busif;
  sc_in<bool>  gnt_linelock, gnt_tagr, gnt_tagw, gnt_bank[CACHE_BANKS], gnt_busif;
  ...
  SC_CTOR(MWritePort) {
    ...
    SC_METHOD(MainMethod);
      ...
      sensitive << port_wr << port_direct << port_bsel << port_rlink_wcond << port_writeback << port_invalidate << port_adr << port_data;
...
protected:
  ...
  // Registers...
  sc_signal<e_writeport_state> state_reg;
  sc_signal<int> state_trace;
  ...
  // Internal signals...
  sc_signal<e_writeport_state> next_state;
...
public:
  ...
  sc_in<TWord> wiadr_busif, wiadr_rp[RPORTS], adr_wp[WPORTS], way_wp[WPORTS];  // (way+index) adresses from various ports;
                                                               // must be kept constant as long as 'req_*' lines are held
  sc_out<TWord> wiadr_bank[CACHE_BANKS][BR_PORTS];   // adress lines to the banks
  ...
  //   Tag RAM...
  //   - Tag RAM must have CPU_CORES ports.
  //   - Each port is reserved for one CPU (three ports: RP #n, RP #CPU_CORES+n, WP #n).
  //   - BUSIF uses port #CPU_CORES-1.
  //   - Arbiter provides:
  ...
  sc_in<bool> req_busif_bank[CACHE_BANKS], req_wp_bank[WPORTS][CACHE_BANKS], req_rp_bank[RPORTS][CACHE_BANKS];
  sc_out<bool> gnt_busif_bank[CACHE_BANKS], gnt_wp_bank[WPORTS][CACHE_BANKS], gnt_rp_bank[RPORTS][CACHE_BANKS];
  ...
  SC_CTOR(MArbiter) {
    ...
    SC_METHOD(LineLockMethod);
      ...
      for (n = 0; n < WPORTS; n++) sensitive << req_wp_linelock[n] << adr_wp[n];
    SC_METHOD(TagMethod);
      sensitive << tagram_ready << req_busif_tagr << req_busif_tagw;
      for (n = 0; n < RPORTS; n++) sensitive << req_rp_tagr[n];
      for (n = 0; n < WPORTS; n++) sensitive << req_wp_tagr[n] << req_wp_tagw[n];
    SC_METHOD(BankMethod);
      sensitive << wiadr_busif;
      for (n = 0; n < RPORTS; n++) sensitive << wiadr_rp[n];
      for (n = 0; n < WPORTS; n++) sensitive << adr_wp[n] << way_wp[n];
      for (b = 0; b < CACHE_BANKS; b++) {
        sensitive << req_busif_bank[b];
        for (n = 0; n < RPORTS; n++) sensitive << req_rp_bank[n][b];
        for (n = 0; n < WPORTS; n++) sensitive << req_wp_bank[n][b];
      }
    SC_METHOD(BusIfMethod);
      for (n = 0; n < RPORTS; n++) sensitive << req_rp_busif[n];
      for (n = 0; n < WPORTS; n++) sensitive << req_wp_busif[n];
    SC_METHOD(SnoopMethod);
      sensitive << next_linelock_reg;
      for (n = 0; n < WPORTS; n++) sensitive << adr_wp[n];
...
protected:
  ...
  sc_signal<sc_uint<RPORTS+WPORTS+1> > bank_reg[CACHE_BANKS];
  ...
  sc_signal<sc_uint<RPORTS+WPORTS+1> > next_bank_reg[CACHE_BANKS];
...
public:
  ...
  sc_in<bool> wp_rlink_wcond [WPORTS];
  sc_out<bool> wp_wcond_ok [WPORTS];
...
protected:
  ...
  // Bank RAM...
  sc_signal<bool> bankram_rd[CACHE_BANKS][BR_PORTS], bankram_wr[CACHE_BANKS][BR_PORTS];
  sc_signal<TWord> bankram_wiadr[CACHE_BANKS][BR_PORTS];
  sc_signal<TWord> bankram_wdata[CACHE_BANKS][BR_PORTS];
  sc_signal<TWord> bankram_rdata[CACHE_BANKS][BR_PORTS];

  // BUSIF...
  sc_signal<EBusIfOperation> busif_op;
  sc_signal<bool> busif_nolinelock, busif_busy;
  sc_signal<bool> busif_tag_rd, busif_tag_wr, busif_bank_rd[CACHE_BANKS], busif_bank_wr[CACHE_BANKS];
  sc_signal<TWord> busif_adr_in, busif_adr_out, busif_data_in[CACHE_BANKS], busif_data_out[CACHE_BANKS];
  sc_signal<bool> busif_data_out_valid[CACHE_BANKS];
  ...
  sc_signal<bool> req_busif_tagw, req_wp_tagw[WPORTS], req_busif_tagr, req_wp_tagr[WPORTS], req_rp_tagr[RPORTS];
  sc_signal<bool> gnt_busif_tagw, gnt_wp_tagw[WPORTS], gnt_busif_tagr, gnt_wp_tagr[WPORTS], gnt_rp_tagr[RPORTS];

  sc_signal<bool> req_busif_bank[CACHE_BANKS], req_wp_bank[WPORTS][CACHE_BANKS], req_rp_bank[RPORTS][CACHE_BANKS];
  sc_signal<bool> gnt_busif_bank[CACHE_BANKS], gnt_wp_bank[WPORTS][CACHE_BANKS], gnt_rp_bank[RPORTS][CACHE_BANKS];
