...
/* Various necessary definitions from or1ksim header files */
...
const bool config_sim_verbose = false;


static struct channel *channel_init (const char *descriptor);
static int channel_open (struct channel *channel);
static int channel_read (struct channel *channel, char *buffer, int size);
static int channel_write (struct channel *channel, const char *buffer, int size);
static void channel_close (struct channel *channel);

static void report_interrupt (int line);
static void clear_interrupt (int line);
...
static void sched_add (void (*job_func) (void *), void *job_param, int32_t job_time, const char *func);
static void sched_find_remove (void (*job_func) (void *), void *dat);


// unused functionality

static inline void vapi_install_handler (unsigned long id, void (*read_func) (unsigned long, unsigned long, void *), void *dat) {}
static inline void vapi_send (unsigned long id, unsigned long data) {}
...
{
  struct dev_16450 *uart = (struct dev_16450 *) dat;
...
uart_check_char (void *dat)
{
  ...
  //if (retval < 0) perror (uart->channel_str);
}
...
uart_sec_start ()
{
  struct dev_16450 *_new = (struct dev_16450 *) malloc (sizeof (struct dev_16450));
...
/****************** Wrapper & Interface functions (GK) **************/


static CUart *lastUart = NULL;
...
static int channel_write (struct channel *channel, const char *buffer, int size) {
  return write (STDOUT_FILENO, buffer, size); // TBD
}
...
static void sched_add (void (*job_func) (void *), void *job_param, int32_t job_time, const char *func) {
  if (lastUart) lastUart->SchedAdd (job_func, job_param, job_time, func);
}

static void sched_find_remove (void (*job_func) (void *), void *dat) {
  if (lastUart) lastUart->SchedFindRemove (job_func, dat);
}


void CUart::SchedAdd (void (*jobFunc) (void *), void *jobParam, int jobTime, const char *jobName) {
  CEvent *ev, **pCur;
  int absTime = curTime + jobTime;
  ...
  //INFOF (("UART: SchedAdd (%i, %s)", ev->time, ev->name));

  pCur = &firstEvent;
  while (*pCur && (*pCur)->time <= jobTime)
    pCur = &((*pCur)->next);
  ev->next = *pCur;
  *pCur = ev;
}
...
void CUart::SchedFindRemove (void (*jobFunc) (void *), void *jobParam) {
  CEvent **pEv, *ev;
  pEv = &firstEvent;

  while (*pEv) {
    ev = *pEv;
    if (ev->func == jobFunc && ev->param == jobParam) {
      *pEv = ev->next;
      //INFOF (("UART: SchedFindRemove (%i, %s)", ev->time, ev->name));
      delete ev;
    }
    else pEv = &((*pEv)->next);
  }
...
int CUart::Simulate (int steps) {
  CEvent *ev, **pEv;

  lastUart = this;

  // Set back all timers to avoid overflows...
  if (curTime >= (1 << 30)) {
    curTime -= (1 << 30);
    for (ev = firstEvent; ev; ev = ev->next)
      ev->time -= (1 << 30);
  }

  // run & remove all events up to the new 'curTime'...
  //INFOF (("UART: Simulate from %i until %i", curTime, curTime + steps));

  curTime += steps;
  if (curTime < 0) curTime = (1 << 30);   // for the case that someone has passed 'INT_MAX' as 'steps'
  pEv = &firstEvent;
  while (firstEvent && firstEvent->time <= curTime) {
    ev = firstEvent;
    firstEvent = ev->next;
    //INFOF (("UART: Exec (%i, %s)", ev->time, ev->name));
    ev->func (ev->param);
    delete ev;
  }
  return firstEvent ? firstEvent->time : INT_MAX;
}
...
static void report_interrupt (int line) {
  if (lastUart) lastUart->SetIrqPending (true);
}


static void  clear_interrupt (int line) {
  if (lastUart) lastUart->SetIrqPending (false);
}
...
void CUart::Init (TWord _baseAdr) {
  baseAdr = _baseAdr;
  irqPending = false;
  firstEvent = NULL;
  curTime = 0;
  ...
  uart = (struct dev_16450 *) malloc (sizeof (struct dev_16450));
  uart->enabled = 1;
  uart->baseaddr = _baseAdr;
  uart->irq = 0;
  uart->uart16550 = 0;
  uart->jitter = 0;
  uart->channel_str = strdup ("/dev/stdout"); // strdup ("xterm:");
  uart->channel = NULL;
  uart->vapi_id = 0;
...
void CUart::WriteByte (TWord adr, TByte val) {
  //INFOF (("UART: write (0x%x) = %02x", adr, val));
  uart_write_byte (adr - baseAdr, val, uart);
}


TByte CUart::ReadByte (TWord adr) {
  TByte val = uart_read_byte (adr - baseAdr, uart);
  //INFOF (("UART: read (0x%x) = %02x", adr, val));
  return val;
}
